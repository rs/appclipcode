...
import { CPQ_SYMBOLS, FIXED_TLD_INDEX, GAPS_BITS_ORDER_LUT, HOST_SYMBOLS, HUFFMAN_TLDS, KNOWN_WORD_INDEX, SPQ_SYMBOLS, } from "./codec-data.js";
const FORMATS = {
...
const FIXED6_INDEX = new Map(Array.from(FIXED6_ALPHABET, (char, index) => [char, index]));
let hostCoder;
let cpqCoder;
let spqCoder;
let tldCoderCache;
...
export class GaloisField {
    ...
    constructor(primitive, size, genBase) {
        ...
        this.expTbl = new Array(size * 2).fill(0);
        this.logTbl = new Array(size).fill(0);
        let x = 1;
...
export class RSEncoder {
    ...
    numParity;
    genPoly = [1];
    constructor(gf, numParity) {
        this.gf = gf;
        this.numParity = numParity;
        this.buildGenerator();
    }
    encode(data) {
        const result = new Array(data.length + this.numParity).fill(0);
...
class SymbolFrequencyTrie {
    ...
    numSymbols;
    maxDepth = 2;
    constructor(data, symbols, filename) {
        this.data = data;
    ...
    getFrequencies(nodeOffset) {
        const frequencies = new Array(this.numSymbols).fill(0);
    ...
    childOffset(parentOffset, symbolIndex) {
        return this.numSymbols * parentOffset + 1 + symbolIndex;
    }
}
class HuffmanCoder {
    codes;
    constructor(freqs, symbols) {
        this.codes = new Array(freqs.length).fill("");
        const leaves = freqs
            .map((freq, index) => ({
            freq,
            symbolIndex: index,
...
class MultiContextHuffmanCoder {
    ...
    encodeWithStartContext(symbols, startContext) {
        let nodeOffset = 0;
        let depth = 0;
    ...
    advanceContext(nodeOffset, depth, symbolIndex) {
...
function loadTrie(filename, symbols) {
    const data = fs.readFileSync(path.join(DATA_DIR, filename));
    return new SymbolFrequencyTrie(data, symbols, filename);
}
function tldHuffmanCoder() {
    if (!tldCoderCache) {
        tldCoderCache = new HuffmanCoder(HUFFMAN_TLDS.map((entry) => entry.freq), HUFFMAN_TLDS.map((entry) => entry.tld));
    }
    return tldCoderCache;
}
export function newGF(primitive, size, genBase) {
    return new GaloisField(primitive, size, genBase);
}
export function newRSEncoder(gf, numParity) {
    return new RSEncoder(gf, numParity);
}
export function compressURL(rawURL) {
    try {
        ensureInit();
    }
    catch (error) {
        throw new Error(`init coders: ${error.message}`);
    }
    let parsed;
    try {
    ...
    if (hasPathOrQuery) {
        try {
            ...
            throw new Error(`encode path/query: ${error.message}`);
        }
    }
    let hostBits = "";
    let hostFormat = 0;
    try {
        ...
        throw new Error(`encode host: ${error.message}`);
    }
    let bits = "1";
...
export function encodePayload(payloadInput) {
    ...
    const version = trimmed.length > 14 ? 1 : 0;
    ...
    const prePerm = new Array(128).fill(false);
    ...
    const output = new Array(129 + zeroCount128).fill(false);
...
function choosePathQueryEncoding(pathValue, query, fragment) {
    const candidates = [];
    try {
...
function encodeTemplatePathQuery(pathValue, query, fragment) {
    if (fragment !== "") {
...
function matchAutoQueryTemplate(pathValue, query) {
...
function encodeAutoQueryTemplateQueryComponent(param, hasMore) {
...
function encodeHost(host, hasPathOrQuery) {
...
function encodeNonTemplatePathQuery(pathValue, query, fragment) {
    let combinedBits = "";
    let combinedError;
    try {
...
function encodeCombinedPathQuery(pathValue, query, fragment) {
...
function encodeSegmentedPathQuery(pathValue, query, fragment) {
    if (fragment !== "") {
...
function buildSegmentedPathItems(pathValue) {
    if (pathValue === "") {
        return [];
    }
...
function encodeSegmentedPathComponent(component, needsTerminator) {
    if (component === "") {
...
function encodeSegmentedQueryComponent(param, hasMore) {
    const separator = param.indexOf("=");
    if (separator < 0) {
...
function encodeULEB128Value(value) {
    if (value === "") {
...
function parseCompressionURL(rawURL) {
    const scheme = "https://";
...
function canonicalizeHost(authority) {
...
function canonicalizeURLComponent(value, kind) {
    let output = "";
...
function rejectsRawURLComponentByte(char, kind) {
    switch (char) {
        case " ":
        case '"':
        case "%":
...
function isAllowedURLComponentByte(char, kind) {
...
function isASCIIAlphaNum(char) {
    return /^[0-9A-Za-z]$/.test(char);
}
function isHexDigit(char) {
    return /^[0-9A-Fa-f]$/.test(char);
}
function writePercentEncodedByte(code) {
    return `%${code.toString(16).toUpperCase().padStart(2, "0")}`;
}
function intToBits(value, bitCount) {
    let bits = "";
...
function blocksToBits(symbols, bitsPerSymbol) {
    const bits = new Array(symbols.length * bitsPerSymbol).fill(false);
...
function toCharSlice(value) {
    return Array.from(value);
}
function shorterBits(current, candidate) {
    if (current === "" || candidate.length < current.length) {
        return candidate;
    }
    return current;
}
function indexOfAny(value, chars) {
    let best = -1;
