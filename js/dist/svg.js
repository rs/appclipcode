import { BG_RADIUS, CAMERA_LOGO_PATHS, CENTER_X, CENTER_Y, DEG2RAD, PHONE_OUTER_PATH, PHONE_SCREEN_PATH, RING_BIT_COUNTS, RING_GAP_ANGLES, RING_RADII, RING_ROTATIONS, STROKE_WIDTH, } from "./svg-data.js";
export function renderSVG(bits, palette, url, codeType) {
    const lines = [];
    ...
    lines.push(`    <circle cx="${formatFloat(CENTER_X)}" cy="${formatFloat(CENTER_Y)}" id="Background" r="${formatFloat(BG_RADIUS)}" style="fill:${palette.background.hex()}"/>`);
    ...
    for (let ring = 0; ring < 5; ring += 1) {
        ...
        gapOffset += count;
        const posState = new Array(count).fill(-1);
        ...
        lines.push(`        <g name="ring-${ring + 1}" transform="rotate(${RING_ROTATIONS[ring]} ${CENTER_X.toFixed(0)} ${CENTER_Y.toFixed(0)})">`);
...
function writeRingArcsFromState(lines, ringIndex, posState, palette) {
    ...
    for (const arc of arcs) {
        ...
        lines.push(`            <path d="M ${formatFloat(ex)} ${formatFloat(ey)} A ${formatFloat(radius)} ${formatFloat(radius)} 0 ${largeArc} 0 ${formatFloat(sx)} ${formatFloat(sy)}" data-color="${arc.dataColor}" style="fill:none;stroke:${strokeColor.hex()};stroke-linecap:round;stroke-miterlimit:10;stroke-width:${formatFloat(STROKE_WIDTH)}px"/>`);
    }
}
function writeLogo(lines, codeType, palette) {
    if (codeType === "nfc") {
